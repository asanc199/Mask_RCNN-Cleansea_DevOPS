...
def create_new_dataset(bg_dataset, objects_dataset):
    bg_objs = obj_selector(bg_dataset,objects_dataset)
    for bg in tqdm((bg_objs), "Creating Dataset"):
        for objs in bg_objs[bg]:
            label = objs[1]
            # 1. Parse JSON and extract points (TODO)
            # 2. Create Polygons / Binary mask for each label (TODO)
            # 3. Check overlapping between each one of them (TODO)
            # 4. Get array of indices of good masks (TODO)
...
def extractMask(bg):
    # For each bg image
    for obj,label in bg:
        # Extract array of pairs and iterate through 
        print(obj,label)
        '''
        data = json.load( open(json_path) )
        assert len(data["shapes"]) != 0, f"Error: no shape in {json_path}"
